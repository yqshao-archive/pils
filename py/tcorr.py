...
@click.option(
    "-te", "--t-end", default=None, type=float, help="ending time, default is None"
)
...
def msd(
    ...
    if len(dataset) == 1 and dataset[0].endswith(".traj"):
        ...
        for restart in dataset[1:]:
            ds = ds.join(load_ds(f"{restart}/cp2k-md", fmt="cp2k")[0:])
...
def hbnet(
    ...
    np.savetxt(
        "persist.dat",
        np.array([np.arange(window) * stride * dt, a_corr, b_corr, p_corr]).T,
    )
    np.savetxt(
        "hbnet.dat",
        np.concatenate(
            [[np.arange(len(all_pop_info)) * stride * dt], np.array(all_pop_info).T],
            axis=0,
        ).T,
    )
