...
include { aseMD } from './tips/nextflow/ase.nf' addParams(publish: "$params.publish/prod/gen$params.gen")

workflow prod_init {
  channel.value(file("$params.publish/models/gen$params.gen/model*/model", type:'dir')) \
...
workflow prod_cont {
  ...
  next = prev + 1
  restart = channel.fromPath("$params.publish/prod/gen$params.gen/$params.md_tag-$prev/*/asemd.traj")
  channel.value(file("$params.publish/models/gen$params.gen/model*/model", type:'dir')) \
    | combine(restart) \
...
params.cp2k_from = '10'
include { cp2kMD; cp2k } from './tips/nextflow/cp2k.nf' addParams(publish: '$params.publish/cp2k-vali')
...
workflow vali_cont {
  ...
  channel.fromPath("$params.publish/cp2k-vali/nvt-${from-10}-${from}ps/*/cp2k-md-1_${2000*from}.restart") \
    | map {it -> \
